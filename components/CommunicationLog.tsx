  ...
  const fetchComms = useCallback(async () => {
    ...
    if (merchantId && linkedLeadId) {
      ...
      const [merchantRes, leadRes] = await Promise.all([
        supabase.from("communications").select("id, type, direction, contact_name, contact_email, contact_phone, subject, body, duration_seconds, logged_at, lead_id, merchant_id").eq("merchant_id", merchantId),
        supabase.from("communications").select("id, type, direction, contact_name, contact_email, contact_phone, subject, body, duration_seconds, logged_at, lead_id, merchant_id").eq("lead_id", linkedLeadId),
      ]);
      ...
      let query = supabase.from("communications").select("id, type, direction, contact_name, contact_email, contact_phone, subject, body, duration_seconds, logged_at, lead_id, merchant_id").order("logged_at", { ascending: false });
